...
void Brothers::Update(DWORD dt, vector<LPGAMEOBJECT>* listMapObj)
{
	this->dt = dt;
	...
	Boomerang2->Update(dt, listMapObj);
	Boomerang1->Update(dt, listMapObj);
	//DebugOut(L"vx%f\n", this->vx);
...
void Brothers::Render()
{
	...
	if (GetState() == BROTHER_STATE_ATTACK_RIGHT|| GetState() == BROTHER_STATE_DIE)
	{
	...
	else if (GetState() == BROTHER_STATE_ATTACK_LEFT|| GetState() == BROTHER_STATE_DIE)
	{
		if (!WaitAttackBoom2)
